...
export type CreateSpotRequest = {
   ...
   * exactly as `POST /api/media/upload-url` returned it in its `url` field. Not a
   * bucket key: `shared::media::is_media_url` checks the origin and the path shape
   * together, so a bare `spots/019f….jpeg` is rejected.
   *
