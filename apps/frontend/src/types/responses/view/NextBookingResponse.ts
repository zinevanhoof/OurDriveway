import type { Booked } from "@/types/domain/spot";
import type { RenterBookingSpotResponse } from "./RenterBookingResponse";

/**
 * `GET /api/view/renter/bookings/next` — the home screen's next-up card.
 *
 * The same rows as {@link RenterBookingResponse} behind a different response: the card
 * renders a title, a zone and the slots, so it is not sent a status, an end instant or a
 * cancel reason. Carries its spot under the same exception.
 *
 * `null` when nothing is coming — an answer, not a 404.
 */
export type NextBookingResponse = {
  id: string;
  spotId: string;
  booked: Booked;
  /** Which car to bring — the one thing here the renter may have forgotten. */
  licensePlate: string;
  /** EUR cents. Read by the detail sheet the card opens, not by the card. */
  amount: number;
  /** The exception. Null while the spot has not been projected here yet. */
  spot: RenterBookingSpotResponse | null;
};
