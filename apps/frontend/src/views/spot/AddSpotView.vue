...
import SpotBasicInfoStep from '@/components/spot/form/SpotBasicInfoStep.vue';
import SpotAddressStep from '@/components/spot/form/SpotAddressStep.vue';
import SpotAvailabilityStep from '@/components/spot/form/SpotAvailabilityStep.vue';
import SpotImagesStep from '@/components/spot/form/SpotImagesStep.vue';
...
const submit = handleSubmit(async (values) => {
    ...
    try {
        ...
        // `refreshSpots` tells MySpotsView to refetch instead of serving the cached list.
...
// EditSpotView, and the shared one in lib/serverErrors) is gone.
    ...
    <DetailLayout title="New listing" @close="router.back()" :show-action="started">
        <template #main>
            <form id="create-spot-form" @submit="submit" class="space-y-4">
                <SpotBasicInfoStep />
                <SpotAddressStep :set-values="setValues" />
                <SpotAvailabilityStep v-model:availability="availability" v-model:slot-errors="slotErrors" />
                <SpotImagesStep v-model:images="images" :image-errors="imageErrors" />
