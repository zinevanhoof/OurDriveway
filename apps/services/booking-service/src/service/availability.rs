...
///
/// `skip` is the booking currently being confirmed, whose own slots must not count
...
pub fn check(
    ...
    for (date, slots) in requested {
        ...
        let own: Vec<_> = skip
            ...
            .map(|s| s.iter().filter_map(span).collect())
...
mod tests {
    ...
    fn an_empty_single_entry_closes_the_day() {
        ...
        let result = check(&availability, &Booked::new(), &want("10:00", "11:00"), None);
        assert!(matches!(result, Err(Rejection::Closed { .. })));
    }
    ...
    fn request_cannot_overlap_itself() {
        ...
        let result = check(&weekly_9_to_17(), &Booked::new(), &requested, None);
        assert!(matches!(result, Err(Rejection::Taken { .. })));
    }

    #[test]
    fn confirm_ignores_the_booking_s_own_slots() {
        // Confirm re-checks against `booked`, which already contains this booking's
        // own hold. Without `skip` a renter could never pay for their own hold.
        let own = taken("10:00", "11:00");
        assert_eq!(
            check(&weekly_9_to_17(), &own, &want("10:00", "11:00"), Some(&own)),
            Ok(60)
        );
        // But someone else's slot in the same map still blocks.
        let mut contested = own.clone();
        contested
            .get_mut(DATE)
            .unwrap()
            .push(slot("11:00", "12:00"));
        let result = check(
            &weekly_9_to_17(),
            &contested,
            &want("11:00", "12:00"),
    ...
    fn rejects_empty_malformed_and_inverted() {
        assert_eq!(
            check(&weekly_9_to_17(), &Booked::new(), &HashMap::new(), None),
    ...
    fn weekday_comes_from_the_date_not_the_local_clock() {
        ...
        assert!(matches!(
            check(&weekly_9_to_17(), &Booked::new(), &tuesday, None),
