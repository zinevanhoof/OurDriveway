...
use crate::{CONFIG, policy::template};
...
impl Mailer {
    ...
    pub async fn send(&self, mail: &Mail, event_id: &Uuid) -> MyResult<()> {
        ...
        .with_template(
            resend_rs::types::EmailTemplate::new(template_id(mail))
...
///
/// Here rather than in `policy/template.rs` because it is the one part of building a
/// mail that reads `CONFIG` — a provider-side identifier, like `mail_from` above it.
