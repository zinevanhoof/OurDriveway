...
///
/// There is no missing column any more. This model used to omit `host`, the
...
pub struct ViewSpot {
    ...
    /// an exact haversine — see `migrations/view/0001_init/up.sql`. lng before lat
...
impl ViewSpotPatch {
    // No `bind`. `AsChangeset` is the `SET` list `ViewSpotRepository::patch` writes, and
    // `Insertable` is the column list `merge` inserts — a `None` is omitted there, which
    // is how the table's `DEFAULT`s for `images`, `active` and `deleted` are reached. See
    // the note in `domain_models::user::user`.
...
mod tests {
    ...
    /// field to [`ViewSpotPatch`] and this stops compiling, which is the reminder that
    /// `merge`'s hand-written column list and binds need it too. `patch` does not: its
    /// `SET` clause is derived from these fields, so it cannot fall behind them.
    ///
    /// `booked` is absent by construction — availability is a query over the booking
    /// rows, not a column on this one. `host` used to be absent for a sharper reason:
