...
//!
//! **None of these models carry their `record<>` link columns.** `spot.host`,
//! `booking.spot`, `booking.renter` and `payout.host` are set by a subquery that
...
//! routinely lands before its host. Writing a pointer unconditionally would be a
