...
--
--   spot     AND (active OR owner_id = $caller)
--   booking  AND (renter_id = $caller OR owner_id = $caller
--                 OR status IN ('reserved','confirmed'))
--   payout   AND owner_id = $caller
--
...
CREATE TABLE app_user (
    ...
    -- The one sensitive column projected here. Every row stays readable — spot-owner
...
CREATE INDEX spot_owner ON spot (owner_id);
...
-- ─── booking ────────────────────────────────────────────────────────────────
-- The renter's and owner's view of a booking, AND the source of availability for
...
-- The availability query and the owner's upcoming list are both (spot_id =, ends_at >),
...
-- The home screen's "earned this week": SUM(amount) WHERE owner_id = me AND created_at
-- >= monday. Composite rather than owner_id alone — same one index to maintain, but it
-- range-scans the week instead of every booking the host has ever had.
CREATE INDEX booking_owner_created ON booking (owner_id, created_at);
...
CREATE INDEX payout_owner ON payout (owner_id, created_at);
