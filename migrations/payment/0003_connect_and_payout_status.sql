...
-- The worker's lookup is by primary key, so no index is added. `payout_owner` from
-- 0001 still serves `total_for`, whose predicate now also names `status` — a filter on
-- three values over one owner's handful of rows, which is not worth widening the index
-- for.
--
