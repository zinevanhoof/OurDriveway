...
CREATE INDEX payment_owner ON payment (owner_id, status);
...
CREATE INDEX booking_owner ON booking (owner_id, status, ends_at);
...
CREATE INDEX payout_owner ON payout (owner_id);
...
-- was needed at all.
--
-- So the lock is taken on the OWNER ID rather than on a row invented to hold it:
--
--   BEGIN;
--   SELECT pg_advisory_xact_lock($1);   -- $1 = low 64 bits of owner_id
